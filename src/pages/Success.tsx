...
const Success = () => {
  ...
  const isInAnalysis = order.status === "EM_ANALISE";
  const isRejected = order.status === "REPROVADO";

  let statusTitle = "Pedido Aprovado!";
  let statusMessage =
    "Seu pedido foi processado com sucesso. Em breve entraremos em contato.";
  let statusIcon = <CheckCircle className="w-12 h-12 text-success" />;
  let statusColor = "text-success";
  let statusBg = "bg-success/10";

  if (isInAnalysis) {
    statusTitle = "Pedido em Análise";
    statusMessage =
      "Seu pedido está em análise. Entraremos em contato em breve com mais informações sobre o status do seu crédito.";
    statusIcon = <Clock className="w-12 h-12 text-amber-500" />;
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-2xl bg-card rounded-lg shadow-elegant-lg p-8 animate-scale-in">
        ...
        <div className="flex justify-center mb-6">
          <div
            className={`w-20 h-20 rounded-full ${statusBg} flex items-center justify-center`}
          >
            {statusIcon}
          </div>
        </div>
        ...
        <div className="text-center mb-8">
          <h1
            ...
            className={cn("font-display text-3xl font-bold mb-2", statusColor)}
          >
            {statusTitle}
          </h1>
